program swimming_plate
  ...
  !
    ...
    real, parameter    :: finish=21
    integer            :: b(3) = [16,16,1]
  !
  ! -- Hyperparameters
    real, parameter    :: thicc=0.03*L
    real, parameter    :: A = 0.1*L, St_d = 0.3, k_x=0., k_z=32.0, h_roughness=0.0
  ...
  ! -- Setup solver
    ...
    !
      time_loop: do while (flow%time<finish/f.and..not.there)
        ...
        if((t>(finish-14)/f).and.(mod(t,0.005/f)<dt)) call flow%write(geom, write_vtr=.false.)
      end do time_loop
