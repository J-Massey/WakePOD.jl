...
def plot_flows(qi, fn, _cmap, lim, fwarp):
    # Test plot
...
def fwarp(pxs):
    return -0.5*(0.28 * pxs**2 - 0.13 * pxs + 0.05) * np.sin(2*np.pi*(0 - (1.42 * pxs)))

def warp_velocity_field(u, pxs):
...
new_u_shifted = warp_velocity_field(utest, pxs)


plot_flows(new_u_shifted.T, "warp_test", "icefire_r", [0., 1.4], fwarp)
