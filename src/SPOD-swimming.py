...
flow = np.load("data/swimming/v.npy")
...
plt.savefig(f"./swimming/figures/testv.pdf", dpi=600)
plt.close()



spod(flatflucs,dt,"swimming",weight='default',nOvlp='default',window='default',method='fast')

SPOD_LPf  = h5py.File(os.path.join('swimming','SPOD_LPf.h5'),'r') # load data from h5 format
...
plt.savefig(f"./swimming/figures/spectrum.pdf", dpi=600)
plt.close()