...
flat_fucks = flucs_field.reshape(3, nx * ny, nt)
...
np.isclose(
    (D1y @ flat_fucks[0]).reshape(nx, ny, nt),
...
big_fucks = flat_fucks.reshape(3 * nx * ny, nt).repeat(4, axis=0)
big_fucks.shape
(L1_big @ big_fucks)
...
te_field = Lq[0, :, :].reshape(nx, ny, nt)
...
anim.save(f"stationary/figures/test_p.gif", fps=50, bitrate=-1, dpi=400)
