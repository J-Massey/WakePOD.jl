...
# v = v[::16, ::16, ::64]
u = np.random.rand(5,7,5)
v = np.random.rand(5,7,5)
...
flat_mean_field = means.reshape(2, nx*ny)
flat_fucks = flucs_field.reshape(2, nx * ny, nt)
...
def test_grad_operator_y(nx, ny):
    ...
    return np.isclose((oprand @ utestrand.reshape(nx*ny, nt)).reshape(nx, ny, nt), durand)
...
D1y = create_grad_operator_y(nx, ny, 1)
np.isclose((D1y@flat_fucks[0]), np.gradient(flucs_field[0], 1, axis=1, edge_order=2))
...
L1 = np.array([(np.diag(-flat_mean_field[0])+LAP/Re), np.diag(-D1x@flat_mean_field[0])])  # u (ax.)
L2 = np.array([Z, (np.diag(-flat_mean_field[1])+LAP/Re), Z])  # v (rad.)
L4 = np.array([-D1x.toarray(), -D1y.toarray(), Z])  #contin.
L  = np.array([L1, L2, L4])
print("The memory size of numpy array arr is:",L.itemsize*L.size/1e9,"GB")

Luprime1 = (L1[0] @ flat_fucks[0, :, 0] + L1[1] @ flat_fucks[1, :, 0]). reshape(nx, ny)
...
(LAP @ flat_fucks[0]).reshape(nx, ny, nt)

gop = (D1x @ (D1x @ flat_fucks[0]) + D1y @ (D1y @ flat_fucks[0])).reshape(nx, ny, nt)
np.isclose(compute_laplacian(flucs_field, dx, dy)[0], gop)

test = np.random.rand(nx, ny, nt)
d2x = np.gradient(np.gradient(flucs_field[0], dx, axis=0, edge_order=2), dx, axis=0, edge_order=2)
d2y = np.gradient(np.gradient(flucs_field[0], dy, axis=1, edge_order=2), dy, axis=1, edge_order=2)
od2x = (D1x @ (D1x @ flat_fucks[0])).reshape(nx, ny, nt)
od2y = (D1y @ (D1y @ flat_fucks[0])).reshape(nx, ny, nt)
np.isclose(od2y, d2y)
...
Luprime = lns_operator(np.repeat(means.reshape(2, nx, ny, 1), nt, axis=3), flucs_field, 10250)
